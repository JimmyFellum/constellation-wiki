...
1. scan `sources/raw/`
2. find sources/raw/ -type f ! -name "README.md" (identify unregistered or changed items)
...
3. append to [[log]] and stop if unreachable
...
If bootstrap finds no new source, changed source, removed source, or unreachable remote blocker, Runtime MUST append a `no-op` entry to `absorb_log.json` and stop without changing [[log]].
